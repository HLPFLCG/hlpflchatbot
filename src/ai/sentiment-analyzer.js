...
export class AISentimentAnalyzer {
  ...
  fallbackAnalysis(message) {
    ...
    // Positive indicators
    const positiveWords = ['great', 'awesome', 'excellent', 'love', 'amazing', 'perfect', 'thank', 'excited'];
    const positiveCount = positiveWords.filter(word => lowerMessage.includes(word)).length;
    
    // Negative indicators
    const negativeWords = ['bad', 'terrible', 'awful', 'hate', 'disappointed', 'frustrated', 'problem', 'issue'];
    ...
    if (lowerMessage.includes('please') || lowerMessage.includes('thank you') || lowerMessage.includes('sir') || lowerMessage.includes('madam')) {
      tone = 'formal';
    } else if (lowerMessage.includes('hey') || lowerMessage.includes('yo') || lowerMessage.includes('sup')) {
      tone = 'casual';
    }
  ...
  async analyzeTrend(conversationHistory) {
    ...
    const lastSentiment = await this.analyzeSentiment(userMessages[userMessages.length - 1].content);
